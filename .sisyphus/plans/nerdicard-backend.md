...
- [ ] 6. Colyseus State Schema
  ...
  **What to do**:
  ...
  - Create `PlayerSchema` (extends `Schema`) with: `sessionId: string`, `displayName: string`, `hp: number` (starts 0), `everGainedHP: boolean` (starts FALSE — set TRUE the first time the player's HP rises above 0 via a successful evaluation; gates Win Condition #1 so that starting HP=0 does not count as an immediate loss. See T13/T14 for usage.), `isConnected: boolean`, `deckFCC: ArraySchema<CardSchema>`, `deckNumber: ArraySchema<CardSchema>`, `deckAction: ArraySchema<CardSchema>`, `discardGraveyard: ArraySchema<CardSchema>` ("3rd Dimension"), `hand: ArraySchema<CardSchema>` annotated with `@view()` — PRIVATE to owner; opponents see only `handCount`. Field `handCount: number` (kept in sync on every add/remove), `availableVariables: ArraySchema<string>` (x1..x10, removed as used), `variableUsagesLeft: number` (count, mirror), `boards: ArraySchema<FunctionBoardSchema>` (max 3), `boardCount: number` (mirror for opponents), `baseFunctionUnlocked: boolean`, `hasUsedVariableThisConstruction: boolean`.
...
- Steps in order:
  ...
  5. `hpGain10 = Math.floor(value * complexity / 10) * 10` — HP stored as integer ×10 (see schema constraint #4). Use `Math.floor` for HP GAIN (loss cases use floor toward zero).
   5b. **HP=0 vulnerability flag** (BUGFIX for rulebook contradiction): whenever `hpGain10 > 0`, set `state.players.get(playerId).everGainedHP = true`. This flag gates Win Condition #1 (see T14) so a non-evaluating player at starting HP=0 is NOT automatically dead — the HP=0 loss path only applies after the player has, at some point, gained positive HP. A "stash and ambush" player who never evaluates is immune to HP-loss but still vulnerable to variable isolation (#2), force-eval domination (#3), and linear-algebra destruction (#4); stalling prevention (T11's 5-consecutive / 20-global counters) eventually forces evaluation, which sets `everGainedHP=true` and breaks the immunity.
...
- Check ordering (top-down short-circuit):
  1. **(a) HP=0**: any player `hp10 <= 0` **AND** `everGainedHP === true` ⇒ that player loses, opponent wins. BUGFIX: starting HP=0 with `everGainedHP=false` is the game-start state, not a loss; a single attack on a non-evaluator must NOT instantly kill them. Once the player has gained positive HP via evaluation (T13 step 5b sets the flag), HP=0 becomes a meaningful loss. Force-eval non-domination halves HP — if this drives a non-evaluator below 0 from 0, `Math.floor(0/2)=0` so no HP delta occurs (they're safe). A non-evaluator is still vulnerable to (b) variable isolation and (c) force-eval domination, which eventually force evaluation flipping `everGainedHP=true`.
